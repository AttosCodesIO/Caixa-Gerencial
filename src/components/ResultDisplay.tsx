...
export function ResultDisplay({ entries, onRemove }: ResultDisplayProps) {
  ...
  const exportPDF = () => {
    ...
    autoTable(doc, {
      head: [['Data Inicial', 'Data Final', 'Índice', 'Valor Original', 'Valor Corrigido', 'Juros']],
      body: tableData,
      startY: 20,
    });
              ...
              <tr key={entry.id} className="hover:bg-neutral-50/50 transition-colors">
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    <span className="font-medium text-neutral-900">{format(parseISO(entry.startDate), 'dd/MM/yy')}</span>
                ...
                <td className="px-6 py-4">
                  <div className="flex flex-col">
                    ...
                    {entry.additionalInterestRate > 0 && <span className="text-xs">+ {entry.additionalInterestRate}% a.m.</span>}
                  </div>
                </td>
                ...
                <td className="px-6 py-4 text-right font-medium text-emerald-600">
                  {entry.status === 'CALCULATING' ? '...' : formatCurrency(entry.correctedValue || 0)}
                </td>
                <td className="px-6 py-4 text-right">
                  {entry.status === 'CALCULATING' ? '...' : formatCurrency(entry.interestValue || 0)}
                  {entry.status === 'SUCCESS' && (
                    <div className="text-xs text-neutral-400">({entry.totalPercentage?.toFixed(2)}%)</div>
                  )}
                </td>
